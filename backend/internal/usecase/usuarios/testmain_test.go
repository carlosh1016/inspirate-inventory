...
// testPool is shared by every test in this package: standing up a fresh
// Postgres container per test would make the suite unbearably slow. Each
// test uses a unique correo (based on the test name) so rows never collide.
...
func TestMain(m *testing.M) {
	...
	if !testing.Short() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "usuarios integration tests: could not start postgres testcontainer, skipping: %v\n", err)
		} else {
			testPool = pool
		}
	}
