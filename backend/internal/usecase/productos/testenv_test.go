...
func newTestEnv(t *testing.T) *testEnv {
	...
	if _, err := pool.Exec(context.Background(),
		`TRUNCATE productos, stock_actual, usuarios, sedes, auditoria RESTART IDENTITY CASCADE`,
	); err != nil {
...
// setStock overwrites the vitrina stock row cantidad for a producto, used to
// exercise the delete-with-stock scenario without going through Tanda 3's
// (not-yet-built) movimientos usecase.
func setStock(t *testing.T, pool *pgxpool.Pool, itemID int64, cantidad string) {
	...
	_, err := pool.Exec(context.Background(),
		...
		cantidad, stockactual.TipoItemProducto, itemID,
	)
	if err != nil {
