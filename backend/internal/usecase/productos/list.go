...
// ListInput is the raw, unvalidated query params for GET /productos.
...
// ListResult is a page of productos plus pagination metadata.
type ListResult struct {
	Items    []generated.ListProductosPaginatedRow
	Total    int64
	Page     int
	PageSize int
}

// List returns a filtered, sorted, paginated page of productos.
func (s *Service) List(ctx context.Context, in ListInput) (ListResult, error) {
	...
	items, total, err := s.Productos.ListPaginated(ctx, repo.ListFilter{
		Page:           page,
		PageSize:       pageSize,
		SortCol:        sortCol,
		SortDir:        sortDir,
